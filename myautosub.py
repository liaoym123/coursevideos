# -*- coding: utf8 -*-
import json,time,sys,re,os,oss2,glob
...
def startSrt(auth,bucket,con,voicefile):
	srtsub = ''
	i=0

	uploadfile = voicefile.split('\\')
	uploadfile=uploadfile[-1]
	subname = uploadfile.split('.')
	subname.pop()
	subname = ".".join(subname)
	uploadfile = subname+".mp3"

	print('正在提取16K音频……')
	os.system("ffmpeg -i "+voicefile+' -ar 16000 -y '+uploadfile)
...
voicefiles = glob.glob(sys.argv[1])
i=0
for voicefile in voicefiles:
	i=i+1
	value = "Thread(target=startSrt, args=(auth,bucket,con,voicefile,), name=\"线程"+str(i)+"\")"
	#str2 = 'thread{} =  {}'.format(i,value)
	exec('thread{} =  {}'.format(i,value))
